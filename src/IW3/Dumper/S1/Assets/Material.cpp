...
namespace ZoneTool
{
	namespace S1
	{
		std::string get_s1_techset(std::string name, std::string matname, bool* result)
		{
			...
			{
				ZONETOOL_ERROR("Could not find mapped S1 techset for IW3 techset \"%s\" (material: %s)", name.data(), matname.data());
				*result = false;
				return "2d";
			}
			*result = true;
		...
		{
			std::uint64_t h1_bits = H1::SURFTYPE_BITS_DEFAULT;
			...
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_BARK, H1::SURFTYPE_BITS_BARK);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_BRICK, H1::SURFTYPE_BITS_BRICK);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_CARPET, H1::SURFTYPE_BITS_CARPET);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_CLOTH, H1::SURFTYPE_BITS_CLOTH);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_CONCRETE, H1::SURFTYPE_BITS_CONCRETE);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_DIRT, H1::SURFTYPE_BITS_DIRT);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_FLESH, H1::SURFTYPE_BITS_FLESH);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_FOLIAGE, H1::SURFTYPE_BITS_FOLIAGE_DEBRIS);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_GLASS, H1::SURFTYPE_BITS_GLASS);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_GRASS, H1::SURFTYPE_BITS_GRASS);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_GRAVEL, H1::SURFTYPE_BITS_GRAVEL);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_ICE, H1::SURFTYPE_BITS_ICE);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_METAL, H1::SURFTYPE_BITS_METAL_SOLID);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_MUD, H1::SURFTYPE_BITS_MUD);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_PAPER, H1::SURFTYPE_BITS_PAPER);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_PLASTER, H1::SURFTYPE_BITS_PLASTER);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_ROCK, H1::SURFTYPE_BITS_ROCK);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_SAND, H1::SURFTYPE_BITS_SAND);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_SNOW, H1::SURFTYPE_BITS_SNOW);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_WATER, H1::SURFTYPE_BITS_WATER_WAIST);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_WOOD, H1::SURFTYPE_BITS_WOOD_SOLID);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_ASPHALT, H1::SURFTYPE_BITS_ASPHALT);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_CERAMIC, H1::SURFTYPE_BITS_CERAMIC);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_PLASTIC, H1::SURFTYPE_BITS_PLASTIC_SOLID);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_RUBBER, H1::SURFTYPE_BITS_RUBBER);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_CUSHION, H1::SURFTYPE_BITS_CUSHION);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_FRUIT, H1::SURFTYPE_BITS_FRUIT);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_PAINTEDMETAL, H1::SURFTYPE_BITS_PAINTEDMETAL);
			convert(IW3::SurfaceTypeBits::SURFTYPE_BITS_SLUSH, H1::SURFTYPE_BITS_SLUSH);
		...
		std::uint8_t get_h1_sortkey(std::uint8_t sortkey, std::string matname, std::string s1_techset, std::string iw3_techset)
		{
		...
		std::uint8_t get_h1_camera_region(std::uint8_t camera_region, std::string matname, std::string s1_techset, std::string iw3_techset)
		{
			if (iw3_techset == "wc_effect")
			{
			...
			if (s1_techset.find("_t0c0") != std::string::npos || s1_techset.find("_b0c0") != std::string::npos)
			{
		...
		void dump(Material* asset)
		{
			if (asset)
			{
				...
				if (asset->techniqueSet)
				{
					...
					bool result = false;
					s1_techset = S1::get_s1_techset(iw3_techset, asset->name, &result);
					if (!result)
					{
				...
				S1::MaterialGameFlags_ s1_game_flags;
				s1_game_flags.packed = iw3_game_flags.packed;
				s1_game_flags.fields.unk7 = iw3_game_flags.fields.unkNeededForSModelDisplay;
				s1_game_flags.fields.unkCastShadowMaybe = iw3_game_flags.fields.MTL_GAMEFLAG_CASTS_SHADOW;

				matdata["gameFlags"] = s1_game_flags.packed;//asset->gameFlags; // convert

				matdata["sortKey"] = S1::get_h1_sortkey(asset->sortKey, asset->name, s1_techset, iw3_techset);
				...
				matdata["surfaceTypeBits"] = S1::convert_surf_type_bits(asset->surfaceTypeBits);
				// hashIndex;

				//matdata["stateFlags"] = asset->stateFlags; // convert ( should be the same )
				matdata["cameraRegion"] = S1::get_h1_camera_region(asset->cameraRegion, asset->name, s1_techset, iw3_techset);
