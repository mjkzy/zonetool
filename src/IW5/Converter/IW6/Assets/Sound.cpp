		...
		void GenerateIW6SoundAlias(snd_alias_t* alias, IW6::snd_alias_t* iw6_alias, ZoneMemory* mem, const std::function<std::string(const char* filename)>& get_streamed_sound_data)
		{
			...
			if (alias->speakerMap)
			{
				...
				for (char x = 0; x < 2; x++)
				{
					for (char y = 0; y < 2; y++)
					{
						iw6_alias->speakerMap->channelMaps[x][y].speakerCount = alias->speakerMap->channelMaps[x][y].entryCount;
						for (char i = 0; i < 6; i++)
						{
			...
			iw6_flags.packed.unknown1 = iw5_flags.unknown1;
			iw6_flags.packed.unknown2 = iw5_flags.unknown2;
